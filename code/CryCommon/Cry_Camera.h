...
public:
	...
	ILINE void CalcScreenBounds(int * vOut, const AABB * pAABB, int nWidth, int nHeight, float fBias, float fOffset) const;
...
ILINE bool CCamera::Project(const Vec3& p, Vec3& result, Vec2i topLeft, Vec2i widthHeight ) const
{
	...
	visible = visible && (cry_fabsf(projected.x) <= 1.0f) && (cry_fabsf(projected.y) <= 1.0f);
...
ILINE void CCamera::CalcScreenBounds(int * vOut, const AABB * pAABB, int nWidth, int nHeight, float fBias, float fOffset) const
{
	...
	Vec3 vPos = GetPosition() + vDir * fOffset; //FUDGE: push away from camera slightly
	...
	vOut[0] = max(0, min(widthHeight.x, (int)x0));
	vOut[1] = max(0, min(widthHeight.y, (int)y0));
	vOut[2] = max(0, min(widthHeight.x, (int)x1));
	vOut[3] = max(0, min(widthHeight.y, (int)y1));
}
...
{	
  ...
  assert (FOV>=0.0000001f && FOV<gf_PI);		  //check if specified FOV is valid
	...
	float projLeftTopY = (f32)((1.0f / cry_tanf(m_fov * 0.5f)) * (fHeight * 0.5f));
...
CAM_NO_INLINE bool CCamera::AdditionalCheck( const AABB& aabb ) const
{
...
CAM_NO_INLINE bool CCamera::AdditionalCheck( const Vec3& wpos, const OBB& obb, f32 uscale ) const
{
