...
enum EAgentAvoidanceAbilities
{
	...
	eAvoidance_Actors			= 0x02,		// Agent can avoid puppets
...
struct AgentMovementAbility
{
	...
	AgentMovementAbility():b3DMove(false),bUsePathfinder(true), usePredictiveFollowing(false), allowEntityClampingByAnimation(false),
		...
		pushableObstacleMassMin(0.0f), pushableObstacleMassMax(0.0f)
...
struct AIObjectParams
{
	...
	// This constructor is helpful for creating Leader object params
	AIObjectParams(uint16 _type, IAIObject* _association, int nGroupID)
		: type(_type)
		, association(_association)
		, name(0)
		, entityID(0)
	{
		m_sParamStruct.m_nGroup = nGroupID;
	}

	AIObjectParams(uint16 _type, const AgentParameters& sParamStruct, const AgentMovementAbility& moveAbility, IAIObject* _association = 0, EntityId _entityID = 0)
...
struct SAIEVENT
{
	...
	SAIEVENT() : bFuzzySight(false), bPathFound(false), nDeltaHealth(0), fThreat(0.f),
...
struct SAIPredictedCharacterStates
{
	...
	enum {maxStates = 32};
	DynArray<SAIPredictedCharacterState> states;
};
...
struct SOBJECTSTATE
{
	...
	float	fDistanceOffFromPath; // Distance between the physical position and the current path position.
...
struct SAIBodyInfo
{
	...
	SAIBodyInfo() : vEyePos(ZERO), vEyeDir(ZERO), vEyeDirAnim(ZERO), vEntityDir(ZERO), vAnimBodyDir(ZERO), vMoveDir(ZERO), vUpDir(ZERO), vFireDir(ZERO), vFirePos(ZERO),
		...
		isAiming(false), isFiring(false), linkedVehicleEntity(0), lean(0.0f), slopeAngle(0.0f),
...
struct IAICommunicationHandler
{
	...
	virtual tSoundID PlayVoice(CommPlayID playID, const char* name, IEventListener* listener = 0) = 0;
