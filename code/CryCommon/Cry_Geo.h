...
struct AABB {
  ...
  ILINE  explicit AABB( float radius ) 
	...
	*/
	...
	ILINE void SetAABBfromOBB(const OBB& obb);
	ILINE static AABB CreateAABBfromOBB(const OBB& obb);
	...
	*  expanded AABB in world-space
	*/
	ILINE void SetAABBfromOBB(const Vec3& wpos, const OBB& obb, f32 scaling = 1.f);
	ILINE static AABB CreateAABBfromOBB(const Vec3& wpos, const OBB& obb, f32 scaling = 1.f);
...
struct OBB
{
	...
	ILINE static OBB CreateOBB(const Matrix33& m33, const Vec3& hlv, const Vec3& center) { OBB obb; obb.m33=m33; obb.h=hlv; obb.c=center; return obb;	}
	...
	ILINE static OBB CreateOBBfromAABB( const Matrix33& m33, const AABB& aabb ) { OBB obb; obb.SetOBBfromAABB(m33,aabb); return obb;	}
	ILINE static OBB CreateOBBfromAABB( const Quat& q, const AABB& aabb ) { OBB obb; obb.SetOBBfromAABB(q,aabb); return obb;	}
...
//create an AABB using just the extensions of the OBB and ignore the orientation. 
ILINE void AABB::SetAABBfromOBB(const OBB& obb)
{
	min = obb.c - obb.h;
	max = obb.c + obb.h;
}

ILINE /* static */ AABB AABB::CreateAABBfromOBB(const OBB& obb)
{
...
*  expanded AABB in world-space
*/
ILINE void AABB::SetAABBfromOBB(const Vec3& wpos, const OBB& obb, f32 scaling /* = 1.f */)
{
...
ILINE /* static */ AABB AABB::CreateAABBfromOBB(const Vec3& wpos, const OBB& obb, f32 scaling /* = 1.f */)
{
	AABB taabb;
	taabb.SetAABBfromOBB(wpos, obb, scaling);
	return taabb;
}
