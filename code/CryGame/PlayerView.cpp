...
void CPlayerView::ViewProcess(SViewParams &viewParams)
{
	...
	else if(m_in.stats_onLadder && !m_in.bIsThirdPerson)
	{
		ViewFirstThirdSharedPre(viewParams);
...
void CPlayerView::ViewPostProcess(CPlayer &rPlayer, SViewParams &viewParams)
{
	...
	assert( rPlayer.m_viewQuat.IsValid() );
	...
	assert( rPlayer.m_baseQuat.IsValid() );
...
void CPlayerView::ViewFirstThirdSharedPre(SViewParams &viewParams)
{
	...
	{
		float t = gEnv->pTimer->GetFrameStartTime().GetSeconds();
		Ang3 orientation(m_io.viewQuatFinal);
		Ang3 keep(g_pGameCVars->cl_camKeepX, g_pGameCVars->cl_camKeepY, g_pGameCVars->cl_camKeepZ);
		Ang3 offset(DEG2RAD(g_pGameCVars->cl_camOffsetX), DEG2RAD(g_pGameCVars->cl_camOffsetY), DEG2RAD(g_pGameCVars->cl_camOffsetZ));
		Ang3 rotate(DEG2RAD(g_pGameCVars->cl_camRotateX), DEG2RAD(g_pGameCVars->cl_camRotateY), DEG2RAD(g_pGameCVars->cl_camRotateZ));
		orientation.x = orientation.x * keep.x + offset.x + rotate.x * t;
		orientation.y = orientation.y * keep.y + offset.y + rotate.y * t;
		orientation.z = orientation.z * keep.z + offset.z + rotate.z * t;
		m_io.viewQuatFinal.SetRotationXYZ(orientation);
		viewParams.position += m_io.viewQuatFinal * Vec3(g_pGameCVars->cl_camTranslateX, g_pGameCVars->cl_camTranslateY, g_pGameCVars->cl_camTranslateZ);
	}
...
void CPlayerView::HandsPostProcess(CPlayer &rPlayer, SViewParams &viewParams)
{
	...
	if (rPlayer.GetEntity()->GetSlotFlags(2) & ENTITY_SLOT_RENDER)
	{
		...
		if (pHands)
		{
			...
			pHands->SetPostProcessParameter(renderLocation, renderLocation, 0, 0.0f, 0x55);
		}
	}
}
