...
bool CPlayerMovementController::RequestMovement( CMovementRequest &request )
{
	...
	if (request.HasMoveTarget())
	{
		...
		state.SetDistanceOffFromPath(request.GetDistanceOffFromPath());
	}
	else if (request.RemoveMoveTarget())
	{
		...
		state.ClearDistanceOffFromPath();
	}
...
static bool ClampTargetPointToCone(bool bUsingAimIK, Vec3 &target, const Vec3 &pos, const Vec3 &dir, float coneAngle)
{
...
bool CPlayerMovementController::UpdateNormal( float frameTime, SActorFrameMovementParams &params )
{
	...
	if(gEnv->bMultiplayer && m_state.HasStance() && m_state.GetStance() == STANCE_PRONE)
	{
		...
		if (gEnv->IsClient() && m_pPlayer->GetGameObject()->IsProbablyVisible())
		{
			Vec3 dir = -m_currentMovementState.entityDirection;
			dir.NormalizeSafe();
			dir.z = 0.0f;
	...
	{
		if (m_state.GetDeltaRotation().z != 0.f)
		{
			volatile int i = 0;
		}
	...
	ClearStrengthJump();
	const SStanceInfo *pPlayerStanceInfo = m_pPlayer->GetStanceInfo(m_pPlayer->GetStance());
	float velocityLength = params.desiredVelocity.GetLength();
	m_desiredSpeed = velocityLength * pPlayerStanceInfo->maxSpeed; //m_pPlayer->GetStanceMaxSpeed( m_pPlayer->GetStance() );
	m_state.RemoveDeltaRotation();
	m_state.RemoveDeltaMovement();
	...
	if (g_pGame->GetCVars()->g_debugaimlook)
	{
		pRend->Draw2dLabel( 10.f, (float)y, 1.5f, white, false,
							"%s:  body=%s   look=%s   lookTarget=(%3.2f, %3.2f, %3.2f)   aim=%s   rotik=%s   move=%s   delta ang=(%3.2f, %3.2f, %3.2f)",
							pEntity->GetName(), bodyTargetType, lookType,
							m_lookTarget.x, m_lookTarget.y, m_lookTarget.z,
							aimType, ikType, moveTargetType,
	...
	//check whether character is outside g_distanceForceNoIK zone
	//should perhaps also take camera fov into account
	bool forceNoIK = m_pPlayer->IsPlayer() ? false : m_pPlayer->GetForceNoIK();
	m_usingAimIK = params.aimIK && !forceNoIK;
	...
	*/
	...
	{
		if( m_pPlayer->GetAnimatedCharacter()->IsAnimationControllingRotation() )
		{
			// Freeze any desired Z-axis rotation while animation is in control
			// to prevent the two from getting out of sync.
			params.deltaAngles[ 2 ] = 0.f;
		}

		if( m_pPlayer->GetAnimatedCharacter()->IsAnimationControllingMovement() )
		{
			// Freeze desired movement while animation is in control, to prevent
			// the two from getting out of sync.
			params.desiredVelocity.Set( 0.f, 0.f, 0.f );
		}
	}
...
void CPlayerMovementController::Serialize(TSerialize &ser)
{
	if(ser.GetSerializationTarget() != eST_Network)	//basic serialization
	{
		...
		ser.Value("bodyDir", m_currentMovementState.entityDirection);
