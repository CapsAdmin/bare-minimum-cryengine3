	...
	public IFireMode
{
	...
	public:
		...
		//IFireMode
		virtual void Init(IWeapon *pWeapon, const IItemParamsNode *params, uint32 id);
...
class CSingleSharedData: public IWeaponSharedData
{
	public:
		...
		virtual void GetMemoryUsage(ICrySizer *s) const
		{
			...
			tracerparams.GetMemoryUsage(s);
			ooatracerparams.GetMemoryUsage(s);
			actions.GetMemoryUsage(s);
			muzzleflash.GetMemoryUsage(s);
			muzzlesmoke.GetMemoryUsage(s);
			muzzlesmokeice.GetMemoryUsage(s);
			reject.GetMemoryUsage(s);
			spinup.GetMemoryUsage(s);
			heatingparams.GetMemoryUsage(s);
			dustparams.GetMemoryUsage(s);
			recoilparamsCopy.GetMemoryUsage(s);
			spreadparamsCopy.GetMemoryUsage(s);
		}

		CSingle::SFireParams			fireparams;
		CSingle::STracerParams		tracerparams;
		CSingle::STracerParams		ooatracerparams;
		CSingle::SSingleActions		actions;
		CSingle::SEffectParamsEx	muzzleflash;
