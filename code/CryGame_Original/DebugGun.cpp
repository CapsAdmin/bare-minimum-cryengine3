		...
		if (pEntity)
		{
			...
			if (pScriptTable)
			{
				...
				if (pScriptTable->GetValue("GetFrozenAmount", func) && func)
				{
					...
					if (frozen > 0.f)
					{
						pRenderer->Draw2dLabel((float)x, (float)(y += dy), (float) font, drawColor, false, "Frozen: %.2f", frozen);
					}
				}
			}
			...
			{
				pRenderer->Draw2dLabel((float)x, y += dy, (float)font, drawColor, false, "%i health", static_cast<int>(pActor->GetHealth()));
			}
