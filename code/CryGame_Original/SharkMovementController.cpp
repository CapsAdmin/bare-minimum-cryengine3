...
bool CSharkMovementController::GetStanceState( const SStanceStateQuery &query, SStanceState &state)
{
	...
	if(query.defaultPose)
	{
		...
		state.weaponPosition = m_pShark->GetWeaponOffsetWithLean(query.stance, query.lean, query.peekOver, m_pShark->GetEyeOffset());
	...
	else
	{
		...
		state.upDirection = bodyInfo.vUpDir;
		state.weaponPosition = tm.TransformPoint(m_pShark->GetWeaponOffsetWithLean(query.stance, query.lean, query.peekOver, m_pShark->GetEyeOffset()));
