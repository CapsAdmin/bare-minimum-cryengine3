...
CVehicleMovementBase::CVehicleMovementBase()
	: m_actorId(0),
	  ...
	  m_pEntity(0),
	  m_maxSpeed(0.f),
	  m_lastPowerForward(0.f),
	  m_lastPowerBackward(0.f),
	  m_enginePos(ZERO),
	  m_runSoundDelay(0.f),
	  m_rpmScale(0.f),
	  m_rpmPitchSpeed(0.f),
...
bool CVehicleMovementBase::Init(IVehicle *pVehicle, const CVehicleParams &table)
{
	...
	if(CVehicleParams soundParams = table.findChild("SoundParams"))
	{
		...
		else
		{
			...
			if(soundParams.haveAttr("eventGroupFunc"))
			{
				functionalityProjectName = soundParams.getAttr("eventGroupFunc");
			}
		}
...
void CVehicleMovementBase::CleanUp()
{
	stl::free_container(m_surfaceSoundInfo);
}
...
void CVehicleMovementBase::Update(const float deltaTime)
{
	...
	if (!pPhysics || !pPhysics->GetStatus(&m_statusDyn) || !pPhysics->GetStatus(&m_statusPos))
	{
		return;
	}

	// Cache current speed
	m_speed = m_statusDyn.v.GetLength();
	m_localSpeed = m_statusDyn.v * m_statusPos.q;
...
void CVehicleMovementBase::OnAction(const TVehicleActionId actionId, int activationMode, float value)
{
	...
	{
		m_lastPowerForward = m_movementAction.power = value;

		if (m_movementAction.power == 0.0f)
		{
			if (m_lastPowerBackward < 0.0f)
			{
				m_movementAction.power = m_lastPowerBackward;
			}
		}
	}
	else if (actionId == eVAI_MoveBack)
	{
		m_lastPowerBackward = m_movementAction.power = -value;

		if (m_movementAction.power == 0.0f)
		{
			if (m_lastPowerForward > 0.0f)
			{
				m_movementAction.power = m_lastPowerForward;
			}
		}
	}
...
void CVehicleMovementBase::EjectionTest(float deltaTime)
{
	...
	const Vec3 zAxis = m_PhysPos.q.GetColumn2();
...
void CVehicleMovementBase::ApplyAirDamp(float angleMin, float angVelMin, float deltaTime, int threadSafe)
{
	...
	Ang3 angles(worldTM);
	Vec3 localW = m_PhysDyn.w * m_PhysPos.q;
...
ISound *CVehicleMovementBase::PlaySound(EVehicleMovementSound eSID, float pulse, const Vec3 &offset, int soundFlags)
{
	...
	if (pSound)
	{
		pSound->GetInterfaceExtended()->SetVolume(m_soundMasterVolume);
	}
...
void CVehicleMovementBase::FreeExhaustSlots()
{
	for (std::vector<SExhaustStatus>::iterator it = m_paStats.exhaustStats.begin(); it != m_paStats.exhaustStats.end(); ++it)
	{
		FreeEmitterSlot(it->boostSlot);
...
SMFXResourceListPtr CVehicleMovementBase::GetEffectNode(int matId)
{
	...
	if(IMaterialEffects *mfx = g_pGame->GetIGameFramework()->GetIMaterialEffects())
	{
