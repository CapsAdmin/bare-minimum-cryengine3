...
class CHitDeathReactions
{
	private:
		...
		struct SCustomAnim
		{
				...
				bool 								IsValid() const
				{
					return !sAnimName.empty();
				}
				void 								SetAnimName(int animID, IAnimationSet *pAnimSet);
				ILINE const string &GetAnimName() const
				{
					return sAnimName;
				}
	...
	public:
		...
		ILINE bool				IsInDeathReaction() const
		{
			return m_currentReaction == eRT_Death;
		}
		ILINE bool				IsInHitReaction() const
		{
			return m_currentReaction == eRT_Hit;
		}
		ILINE bool				IsInReaction() const
		{
			return (IsInDeathReaction() || IsInHitReaction());
		}

		ILINE bool				AreReactionsForbidden() const
		{
			return (m_reactionFlags & SReactionParams::ReactionsForbidden) != 0;
		}

		ILINE ProfileId		GetProfileId() const
		{
			return m_profileId;
		}

		// FIXME: Currently used to prevent AI from moving while in a hit reaction - To be replaced by better method.
		ILINE bool				CanActorMove() const
		{
			return (eAIP_NotPaused == m_AIPausedState);
		}
	...
	private:
		...
		ILINE bool							DeathReactionEndsInRagdoll() const
		{
			return (!g_pGameCVars->g_hitDeathReactions_disableRagdoll && !(m_reactionFlags & SReactionParams::NoRagdollOnEnd));
		}

		void 										SetInReaction(EReactionType reactionType);
		ILINE void 							SetExecutionType(EExecutionType executionType)
		{
			m_currentExecutionType = executionType;
		}
		ILINE bool 							IsExecutionType(EExecutionType executionType) const
		{
			return (m_currentExecutionType == executionType);
		}
		bool										EndCurrentReactionInternal(bool bForceRagdollOnHit, bool bFinished);
		...
		ILINE bool							DoesAnimEventWantToSleepRagdoll(const AnimEventInstance &event) const
		{
			return (event.m_CustomParameter && (strcmp("sleep", event.m_CustomParameter) == 0));
		}
		void										SleepRagdoll();
