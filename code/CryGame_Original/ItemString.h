...
namespace SharedString
{
		...
		public:
			...
			SNameEntry *GetEntry( const char *str )
			{
				...
				if (!pEntry)
				{
					...
					memcpy( const_cast<char *>(pEntry->GetStr()), str, nLen + 1 );
					// put in map.
			...
			void Dump()
			{
				CryLogAlways("NameTable: %d entries", (int32)m_nameMap.size());
	...
	class	CSharedString
	{
		public:
			...
			{
				return GetGameNameTable()->FindEntry(str) != 0;
			}
			...
			{
				GetGameNameTable()->Dump();
			}

		private:
			static CGameNameTable *GetGameNameTable()
			{
				static CGameNameTable table;
				return &table;
			}
			...
			void _release( const char *pBuffer )
			{
				...
				{
					if (CGameNameTable *pNT = GetGameNameTable())
					{
	...
	inline CSharedString::CSharedString( const char *s, bool bOnlyFind )
	{
		...
		if (*s) // if not empty
		{
			SNameEntry *pNameEntry = GetGameNameTable()->FindEntry(s);
	...
	inline CSharedString	&CSharedString::operator=( const char *s )
	{
		...
		if (s && *s) // if not empty
		{
			pBuf = GetGameNameTable()->GetEntry(s)->GetStr();
		}
		else if (s == 0)
		{
			; // debugging here
