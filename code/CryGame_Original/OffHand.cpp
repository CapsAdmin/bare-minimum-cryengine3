...
-------------------------------------------------------------------------
History:
- 12:04:2006   17:22 : Created by Marcio Martins
		...
		public:
			...
			void execute(CItem *cItem)
			{
				...
				if(pMainHand && !pMainHand->IsDualWield())
				{
					...
					timeDelay = (pMainHand->GetCurrentAnimationTime( eIGS_FirstPerson) + 50) * 0.001f;
				}
...
void COffHand::PostSerialize()
{
	...
	{
		EntityId id = m_heldEntityId;
		ThrowObject(eAAM_OnPress);
		ThrowObject(eAAM_OnRelease);
		IgnoreCollisions(false, id);
	}
	else if(m_currentState & (eOHS_HOLDING_GRENADE | eOHS_THROWING_GRENADE))
	{
		m_currentState = eOHS_INIT_STATE;
		m_iPostSerReset = 3;
	}
	...
	else if((m_heldEntityId || m_preHeldEntityId) && m_currentState != eOHS_TRANSITIONING)
	{
		...
		if(m_heldEntityId)
		{
			...
			if(!pEntity)
			{
				...
				if(pPlayer)
				{
					(static_cast<CPlayer *>(pPlayer))->NotifyObjectGrabbed(false, 0, 0);
					pPlayer->SelectNextItem(1, false);
				}
			}
		}
	...
	{
		if(m_currentState == eOHS_TRANSITIONING)
		{
			m_iPostSerReset = 3;
		}
...
void COffHand::UpdateFPView(float frameTime)
{
	...
	if ( pManager )
	{
		pManager->UpdatePickupMessage(m_usable != 0 && !m_IsLadder);
	}
	...
	if(m_iPostSerReset)
	{
		...
		if(m_iPostSerReset <= 0)
		{
			m_iPostSerReset = 0;
			CPlayer *pOwner = static_cast<CPlayer *>(GetOwnerActor());

			if(pOwner && pOwner->GetLinkedVehicle())
			{
				return;
			}

			CItem			*pMain = GetActorItem(pOwner);
			CWeapon		*pMainWeapon = static_cast<CWeapon *>(pMain ? pMain->GetIWeapon() : NULL);

			if(pMainWeapon)
			{
				pMainWeapon->SetBusy(false);
				pMainWeapon->SetActionSuffix("");
				pMainWeapon->PlayAction(g_pItemStrings->idle);
			}

			if (pOwner->GetActorClass() == CPlayer::GetActorClassType())
			{
				pOwner->NotifyObjectGrabbed(false, 0, false);
			}
...
void COffHand::UpdateCrosshairUsabilitySP()
{
	...
	if (pActor)
	{
		...
		if (CanPerformPickUp(pActor) || (isLadder && !onLadder))
		{
			m_usable = true;
		}
		else if (m_usable)
...
void COffHand::UpdateHeldObject()
{
	...
	if(pEntity)
	{
		...
		if(pEntity->GetPhysics())
		{
			...
			pab.bCheckCollisions = 0;	// will effectively disable ragdoll simulation
			pEntity->GetPhysics()->SetParams(&pab);
		}
...
bool COffHand::ProcessOffHandActions(EOffHandActions eOHA, int input, int activationMode, float value /*= 0.0f*/)
{
	...
	else if(eOHA == eOHA_THROW_GRENADE)
	{
		...
		{
			PerformThrowPlayer(activationMode, 0, GetCurrentFireMode());
		}
	}
	...
	else if(eOHA == eOHA_USE)
	{
		if(m_currentState & eOHS_INIT_STATE)
		{
			int typ = CanPerformPickUp(GetOwnerActor(), true);
...
bool COffHand::PreExecuteAction(int requestedAction, int activationMode, bool forceSelect)
{
	...
	if(pMainWeapon)
	{
		exec &= !pMainWeapon->IsBusy() && !pMainWeapon->IsModifying() && !pMainWeapon->IsReloading();
	}

	if(pMainWeapon && (pMainWeapon->IsZoomed() || pMainWeapon->IsZoomingInOrOut()) && (requestedAction == eOHA_THROW_GRENADE))
	{
		...
		case eOHA_RESET:					//Reset main weapon status and reset offhand
			{
				...
				else if(!m_mainHand->IsDualWield() && !m_prevMainHandId)
				{
					...
					timeDelay = (m_mainHand->GetCurrentAnimationTime( eIGS_FirstPerson) + 50) * 0.001f;
				}
...
void COffHand::StartSwitchGrenade(bool xi_switch, bool fakeSwitch)
{
	...
	if(m_mainHandWeapon && (m_mainHandWeapon->IsZoomed() || m_mainHandWeapon->IsZoomingInOrOut()))
	{
	...
	if(m_mainHand && (m_mainHand->TwoHandMode() != 1) && !m_mainHand->IsDualWield())
	{
		...
		if(m_mainHandWeapon && m_mainHandWeapon->IsWeaponRaised())
		{
			...
			m_mainHandWeapon->SetDefaultIdleAnimation( eIGS_FirstPerson, g_pItemStrings->idle);
		}
...
void COffHand::PerformThrowPlayer(int activationMode, EntityId throwableId, int oldFMId /* = 0 */, bool isLivingEnt /*=false*/)
{
	...
	else if (activationMode == eAAM_OnRelease && m_nextGrenadeThrow <= 0.0f)
	{
		...
		{
			GetScheduler()->TimerAction(GetCurrentAnimationTime( eIGS_FirstPerson), CSchedulerAction<FinishGrenadeAction>::Create(FinishGrenadeAction(this, m_mainHand)), false);
		}
	}
}
...
int COffHand::CanPerformPickUp(CActor *pActor, bool getEntityInfo /*= false*/)
{
	...
	EntityId lookAtEntityId = pWorldQuery->GetLookAtEntityId();
	IEntity *pEntity = gEnv->pEntitySystem->GetEntity( lookAtEntityId );
	...
	//if (pMC)
	{
		...
		if((pPhysicalEntity->GetType() == PE_RIGID)
				&& pEntity->GetClass()
				&& (
					!strcmp(pEntity->GetClass()->GetName(), "Default")
					||
					!strcmp(pEntity->GetClass()->GetName(), "Breakage")
				)
		  )
		{
		...
		if (lenSqr < m_range * m_range)
		{
			...
			if(CActor *pActorAI = static_cast<CActor *>(gEnv->pGame->GetIGameFramework()->GetIActorSystem()->GetActor(pEntity->GetId())))
			{
				...
				if(pEntity->GetAI() && pActor->GetEntity() && !pActorAI->GetLinkedVehicle())
				{
					...
					return (pActorAI->GetActorSpecies() != eGCT_UNKNOWN) &&
						   (pActorAI->GetHealth() > 0.f) && !pActorAI->IsFallen() &&
						   !pEntity->GetAI()->IsFriendly(pActor->GetEntity()->GetAI(), false)
						   ? OH_GRAB_NPC
						   : OH_NO_GRAB;
				}
			...
			if(m_hasHelper && pPhysicalEntity && (pPhysicalEntity->GetType() == PE_RIGID))
			{
				SmartScriptTable props;
			...
			//4. Pick boid object
			if (pPhysicalEntity  && (pPhysicalEntity->GetType() == PE_PARTICLE || pPhysicalEntity->GetType() == PE_ARTICULATED) && m_hasHelper)
			{
				return OH_GRAB_OBJECT;
			}
			...
			if(pPhysicalEntity && (pPhysicalEntity->GetType() == PE_RIGID) && !strcmp(pEntity->GetClass()->GetName(), "rock"))
			{
			...
			if (pPhysicalEntity && (pPhysicalEntity->GetType() == PE_RIGID) && pEntityScript && pEntityScript->GetValue("Properties", props))
			{
				int pickable = 0;
				int usable = 0;
		...
		else if (pPhysicalEntity && (pPhysicalEntity->GetType() == PE_STATIC))
		{
...
int COffHand::CheckItemsInProximity(Vec3 pos, Vec3 dir, bool getEntityInfo)
{
	...
	if(IEntity *pEntity = m_pEntitySystem->GetEntity(nearItemId))
	{
		...
		Vec3 itemPos = bbox.GetCenter();
		Vec3 dirToItem = (pos - itemPos);
		IPhysicalEntity *phys = pEntity->GetPhysics();
		ray_hit hit;

		if(!gEnv->pPhysicalWorld->RayWorldIntersection(itemPos, dirToItem, ent_static | ent_rigid | ent_sleeping_rigid,
		...
		for (TGrabTypes::const_iterator i = m_grabTypes.begin(); i != end; ++i, ++m_grabType)
		{
			...
			for(int n = 0; n < pEntity->GetSlotCount(); n++)
			{
				...
				if (ok && slotInfo.pStatObj)
				{
					...
					for(int j = 0; j < 2; j++)
					{
						CryFixedStringT<128> helper; //Should be more than enough space
						helper = "";
				...
				else if(ok && slotInfo.pCharacter)
				{
					...
					if(pAM)
					{
						...
						else
						{
							...
							const int16 id = pSkeletonPose->GetJointIDByName((*i).helper.c_str());

							if (id > -1)
							{
								m_holdOffset = Matrix34(pSkeletonPose->GetAbsJointByID(id).q);
								m_holdOffset.SetTranslation(pSkeletonPose->GetAbsJointByID(id).t);
								m_holdOffset.OrthonormalizeFast();
								m_holdOffset.InvertFast();
								m_hasHelper = true;
								return;
...
void COffHand::StartPickUpItem()
{
	...
	if(!pPlayer->CheckInventoryRestrictions(pPreHeldEntity->GetClass()->GetName()))
	{
		...
		g_pGame->GetIGameFramework()->GetIGameplayRecorder()->Event(GetOwner(), GameplayEvent(eGE_ItemExchanged, 0, 0, 0));
...
void COffHand::PickUpObject(bool isLivingEnt /* = false */)
{
	...
	RequireUpdate(eIUS_General);
	g_pGame->GetIGameFramework()->GetIGameplayRecorder()->Event(GetOwnerActor()->GetEntity(), GameplayEvent(eGE_EntityGrabbed, 0, 0, (void *)m_preHeldEntityId));
}
...
void COffHand::ThrowObject(int activationMode, bool isLivingEnt /*= false*/)
{
	...
	PerformThrowPlayer(activationMode, m_heldEntityId, m_lastFireModeId, isLivingEnt);
...
void COffHand::ThrowNPC(bool kill /*= true*/)
{
	...
	if(kill)
	{
		...
		{
			float prevHealth = pActor->GetHealth();
			float health = prevHealth - 100;
...
void COffHand::RunEffectOnGrabbedNPC(CActor *pNPC)
{
	...
	if(m_grabbedNPCSpecies == eGCT_HUMAN && (pNPC->GetHealth() < m_grabbedNPCInitialHealth))
	{
...
//==========================================================================================
void COffHand::MeleeAttack(bool bShort /* = false */)
{
	if(m_melee)
	{
...
int COffHand::CanExchangeWeapons(IItem *pItem, IItem **pExchangeItem)
{
	...
	{
		//Can not carry more heavy/medium weapons
		IItem *pExItem = GetExchangeItem(pPlayer);

		if(pExItem)
		{
			*pExchangeItem = pExItem;
...
EntityId COffHand::SpawnRockProjectile(IRenderNode *pRenderNode)
{
	...
	pRock->SetParams(GetOwnerId(), GetEntityId(), GetEntityId(), 75, 0);
...
//==============================================================
void COffHand::AttachGrenadeToHand(int grenade)
{
...
//==============================================================
void COffHand::ForcePickUp(EntityId pickupId)
{
	if(m_currentState & (eOHS_INIT_STATE | eOHS_HOLDING_OBJECT | eOHS_PICKING | eOHS_THROWING_OBJECT | eOHS_MELEE))
	{
		m_heldEntityId = pickupId;
		IgnoreCollisions(false, m_heldEntityId);
		DrawNear(false);
		//Do grabbing again
		m_currentState = eOHS_INIT_STATE;
