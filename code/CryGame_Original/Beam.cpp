...
-------------------------------------------------------------------------
History:
- 19:12:2005   12:10 : Created by Marcio Martins
...
//------------------------------------------------------------------------
void CBeam::Update(float frameTime, uint32 frameId)
{
	...
	{
		keepUpdating = true;
		const int slot = m_pWeapon->GetStats().fp ?  eIGS_FirstPerson :  eIGS_ThirdPerson;
		...
		if (m_spinUpTimer > 0.0f)
		{
			...
			m_spinUpTimer = 0.0f;
			m_pWeapon->PlayAction(m_pShared->beamactions.blast.c_str());
			m_fireLoopId = m_pWeapon->PlayAction(m_pShared->actions.fire.c_str(), 0, true);
			...
			SpinUpEffect(false);
			m_effectId = m_pWeapon->AttachEffect(slot, 0, true, m_pShared->effectparams.effect[id].c_str(),
												 m_pShared->effectparams.helper[id].c_str());
		}
		...
		if(m_spinUpTimer == 0.0f)
		{
			if (m_pShared->fireparams.ammo_type_class && m_pShared->fireparams.clip_size >= 0)
			{
				if (m_ammoTimer > 0.0f)
				{
					...
					if (m_ammoTimer <= 0.0f)
					{
						m_ammoTimer = m_pShared->beamparams.ammo_tick;
						//Decrease ammo count
						ammo = m_pShared->fireparams.ammo_type_class;
						...
						ammoCount = ammoCount - m_pShared->beamparams.ammo_drain;
						...
						else
						{
							...
							{
								Reload(m_pWeapon->IsZoomed() || m_pWeapon->IsZoomingInOrOut());

								if (m_pWeapon->IsServer())
								{
									m_pWeapon->SvSetReloadState(CWeapon::eNRS_StartReload);
								}
							}
						}
					}
		...
		bool hitValid = false;
		ray_hit rayhit;
		Vec3 hit = GetProbableHit(m_pShared->beamparams.range, &hitValid, &rayhit);
		...
		if (m_effectId)
		{
			...
			if(m_viewFP != currentView)
			{
				...
				m_pWeapon->AttachEffect(m_viewFP ? eIGS_ThirdPerson : eIGS_FirstPerson, m_effectId, false);
				m_effectId = m_pWeapon->AttachEffect(m_viewFP ? eIGS_FirstPerson : eIGS_ThirdPerson,
													 0, true, m_pShared->effectparams.effect[id].c_str(),	m_pShared->effectparams.helper[id].c_str());
			...
			if(m_pShared->effectparams.scale[id] < 1.0f)
			{
				Quat rot(Quat::CreateRotationVDir(dir));
				rot.Normalize();
				float scaleFX = (((hit - pos).len()) * m_pShared->effectparams.scale[id]);
		...
		if (hitValid)
		{
			if (!m_lastHitValid)
			{
				m_hitSoundId = m_pWeapon->PlayAction(m_pShared->beamactions.hit.c_str(), 0, true, CItem::eIPAF_Default | CItem::eIPAF_ForceThirdPerson | CItem::eIPAF_SoundLooped | CItem::eIPAF_SoundStartPaused);
				...
				if (!m_pShared->hitbeameffectparams.effect[id].empty())
				{
					m_pWeapon->AttachEffect(0, m_effectId, false);
					m_effectId = m_pWeapon->AttachEffect(slot, 0, true, m_pShared->hitbeameffectparams.effect[id].c_str(), m_pShared->hitbeameffectparams.helper[id].c_str());
				}
			}
			...
			if (!m_pShared->beamparams.hit_decal.empty())
			{
				//if (!m_lastHitValid)
			...
			if (!m_pShared->beamparams.hit_effect.empty())
			{
				IParticleEffect *pParticleEffect = gEnv->pParticleManager->FindEffect(m_pShared->beamparams.hit_effect.c_str());

				if (pParticleEffect)
				{
					pParticleEffect->Spawn(true, IParticleEffect::ParticleLoc(rayhit.pt, rayhit.n, m_pShared->beamparams.hit_effect_scale));
				}
			}
			...
			if (m_tickTimer > 0.0f)
			{
				...
				{
					Tick(rayhit, dir);
					m_tickTimer = m_pShared->beamparams.tick;
		...
		{
			if (!m_pShared->hitbeameffectparams.effect[id].empty() && m_lastHitValid)
			{
				m_pWeapon->AttachEffect(0, m_effectId, false);
				m_effectId = m_pWeapon->AttachEffect(slot, 0, true, m_pShared->effectparams.effect[id].c_str(),
													 m_pShared->effectparams.helper[id].c_str());
...
void CBeam::ResetParams(const struct IItemParamsNode *params)
{
	if(!m_fireParams->Valid())
	{
		...
		m_pShared->beamparams.Reset(beam);
		m_pShared->effectparams.Reset(effect);
		m_pShared->hitbeameffectparams.Reset(hiteffect);
		m_pShared->beamactions.Reset(actions);
	}
}
...
void CBeam::PatchParams(const struct IItemParamsNode *patch)
{
	if(!m_fireParams->Valid())
	{
		...
		m_pShared->beamparams.Reset(beam, false);
		m_pShared->effectparams.Reset(effect, false);
		m_pShared->hitbeameffectparams.Reset(hiteffect, false);
		m_pShared->beamactions.Reset(actions, false);
	}
}

//------------------------------------------------------------------
void CBeam::InitSharedParams()
{
	CWeaponSharedParams *pWSP = m_pWeapon->GetWeaponSharedParams();
	assert(pWSP);
	m_fireParams	= pWSP->GetFireSharedParams("BeamData", m_fmIdx);
}

//-----------------------------------------------------------------------
void CBeam::CacheSharedParamsPtr()
{
	CSingle::CacheSharedParamsPtr();
	m_pShared			= static_cast<CBeamSharedData *>(m_fireParams.get());
}
...
void CBeam::StartFire()
{
	...
	m_lastHitValid = false;
	m_firing = true;
	m_spinUpTimer = m_pShared->fireparams.spin_up_time;
	m_tickTimer = m_pShared->beamparams.tick;
	m_ammoTimer = m_pShared->beamparams.ammo_tick;
	m_fired = true;			//For recoil
	SpinUpEffect(true);
	m_pWeapon->PlayAction(m_pShared->actions.spin_up.c_str(), 0, false, CItem::eIPAF_Default | CItem::eIPAF_CleanBlending);
	m_pWeapon->RequireUpdate(eIUS_FireMode);
...
void CBeam::StopFire()
{
	...
	if (m_effectId)
	{
		m_pWeapon->AttachEffect( eIGS_FirstPerson, m_effectId, false);
		m_effectId = 0;
	}
	...
	m_pWeapon->PlayAction(m_pShared->actions.spin_down.c_str(), 0, false, CItem::eIPAF_Default | CItem::eIPAF_CleanBlending);
...
void CBeam::DecalLine(const Vec3 &org0, const Vec3 &org1, const Vec3 &hit0, const Vec3 &hit1, float step)
{
	...
	while(steps--)
	{
		...
		Vec3 dir = (hit - org).normalized() * m_pShared->beamparams.range;
		ray_hit rayhit;
...
void CBeam::Decal(const ray_hit &rayhit, const Vec3 &dir)
{
	...
	decal.fSize = m_pShared->beamparams.hit_decal_size;

	if (m_pShared->beamparams.hit_decal_size_min != 0.f)
	{
		decal.fSize -= Random() * max(0.f, m_pShared->beamparams.hit_decal_size - m_pShared->beamparams.hit_decal_size_min);
	}

	decal.fLifeTime = m_pShared->beamparams.hit_decal_lifetime;
	strcpy(decal.szMaterialName, m_pShared->beamparams.hit_decal.c_str());
...
{
	if (m_pShared->fireparams.damage == 0)
	{
		return;
	}
	...
	if (pEntity)
	{
		...
		HitInfo info(m_pWeapon->GetOwnerId(), pEntity->GetId(), m_pWeapon->GetEntityId(), (float)m_pShared->fireparams.damage, 0.25f,
					 pGameRules->GetHitMaterialIdFromSurfaceId(hit.surface_idx), hit.partid, pGameRules->GetHitTypeId(m_pShared->fireparams.hit_type.c_str()),
					 hit.pt, dir, hit.n);
...
void CBeam::GetMemoryUsage(ICrySizer *s) const
{
	s->Add(*this);
	...
	{
		m_pShared->beamparams.GetMemoryUsage(s);
		m_pShared->beamactions.GetMemoryUsage(s);
		m_pShared->effectparams.GetMemoryUsage(s);
	}
}
