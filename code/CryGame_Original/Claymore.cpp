		...
		case ENTITY_EVENT_ENTERAREA:
			{
				...
				IActor *pActor = g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor((EntityId)event.nParam[0]);	// only detonate for actors...

				if(!pActor)
				{
					pVehicle = g_pGame->GetIGameFramework()->GetIVehicleSystem()->GetVehicle((EntityId)event.nParam[0]);    // ...or vehicles
				}
...
void CClaymore::SetParams(EntityId ownerId, EntityId hostId, EntityId weaponId, int damage, int hitTypeId, float damageDrop, float damageDropMinR)
{
