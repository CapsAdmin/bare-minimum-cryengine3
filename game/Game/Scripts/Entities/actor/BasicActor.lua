...
ActorShared =
{
	...
	splash_sounds =
	{
		{"Sounds/physics:foleys/player:bodyfall_water_deep", SOUND_DEFAULT_3D, 200, 5, 60},
	},
...
function BasicActor:UpdateSounds(frameTime)
	...
	if (newStance ~= oldStance) then
		...
		if (newStance == STANCE_CROUCH or newStance == STANCE_STEALTH) then
			stanceSound = "sounds/physics:foleys/player:crouch_on";
		elseif (newStance == STANCE_PRONE) then
			stanceSound = "sounds/physics:foleys/player:crouch_on";
		else
			--standup
			if (oldStance == STANCE_PRONE) then
				stanceSound = "sounds/physics:foleys/player:crouch_off";
			else
				stanceSound = "sounds/physics:foleys/player:crouch_off";
			end
		end
...
function BasicActor:DoBloodPool()
	...
	if (hits > 0 and splat) then
		...
		Particle.CreateMatDecal(splat.pos, splat.normal, s, 300, self.bloodSplatGround[i], math.random()*360, vecNormalize(dir), splat.entity and splat.entity.id, splat.renderNode, splat.partid, 4);
	...
	if (string.find(hit.type,"bullet")) then
		...
		if(hit.damage > 10) then
			if(self.id == g_localActorId) then
				local sound;
				if (armor > 10) then
					sound="sounds/physics:bullets/impacts:flesh_p";
				else
					sound="sounds/physics:bullets/impacts:flesh_p";
				end
...
function BasicActor:OnAfterShoot(weapon, pos, dir, impulse, ammo)
	...
	if (ammo and ammo.class == "ExplosiveGrenade") then
		...
		--self.aiType = 150;
		CryAction.RegisterWithAI(ammo.id, 150);
	end	
end
...
function BasicActor:SetupTerritoryAndWave(territory, wave)
