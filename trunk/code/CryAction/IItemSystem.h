...
struct IInventory: public IGameObjectExtension
{
  ...
  virtual void RemoveAllItems() = 0;
	...
	virtual void RMIReqToServer_AddAmmo           ( const char* _pszAmmoClass, int _iAmount ) const = 0;
	virtual void RMIReqToServer_RemoveAllAmmo     () const = 0;
	virtual void RMIReqToServer_AddEquipmentPack  ( const char* _pszEquipmentPack ) const = 0;
	...
	virtual IEntityClass* GetTossItem() const = 0;
	virtual void SetTossItem(IEntityClass* tossItem) = 0;

	virtual void SetLastItem(EntityId itemId) = 0;
	virtual EntityId GetLastItem() const = 0;
	virtual unsigned int GetLastSelectedInSlot(IInventory::EInventorySlots slotId) const = 0;
...
UNIQUE_IFACE struct IItemSystem
{
	...
	virtual EntityId GiveItem(IActor *pActor, const char *item, bool sound, bool select, bool keepHistory, const char *setup = NULL) = 0;
