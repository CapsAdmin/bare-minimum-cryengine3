...
-------------------------------------------------------------------------
History:
- 05:8:2007:		Created by Marcio Martins
...
//------------------------------------------------------------------------
void CWorkOnTarget::Init(IWeapon *pWeapon, const struct IItemParamsNode *params, uint32 id)
{
...
void CWorkOnTarget::GetMemoryUsage(ICrySizer *s) const
{
	s->Add(*this);
	s->Add(m_name);
	m_workparams.GetMemoryUsage(s);
	m_workactions.GetMemoryUsage(s);
}
