...
struct CShotgun::SliderBack
{
	...
	void execute(CItem *_this)
	{
		_this->StopLayer(shotgun->m_pShared->fireparams.slider_layer);
	}
};
...
void CShotgun::ResetParams(const struct IItemParamsNode *params)
{
	if(!m_fireParams->Valid())
	{
		...
		m_pShared->shotgunparams.Reset(shotgun);
	}
}
...
void CShotgun::PatchParams(const struct IItemParamsNode *patch)
{
	if(!m_fireParams->Valid())
	{
		...
		m_pShared->shotgunparams.Reset(shotgun, false);
	}
}
...
//-----------------------------------------------------------------------
void CShotgun::CacheSharedParamsPtr()
{
