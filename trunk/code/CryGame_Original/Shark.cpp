...
void CShark::SetStartPos(const Vec3 &targetPos)
{
	...
	float maxLevel = gEnv->p3DEngine->GetAccurateOceanHeight( m_startPos) - 2;
...
void CShark::UpdateStatus(float frameTime, const IEntity *pTarget)
{
	...
	float waterLevel = gEnv->p3DEngine->GetAccurateOceanHeight( myPos);
	...
	else
	{
		...
		if(pTarget)
		{
			...
			if( phys && phys->GetStatus(&targetdyn) && m_stats.speed > 0.5f && targetdyn.v.GetLengthSquared() > 1)
			{
				...
				float dotProd = (dotvel + dotOrient) / 2;
				float relspeed = (targetdyn.v - m_stats.velocity).GetLength();
				desiredSpeed += dotProd * relspeed;
...
void CShark::FindEscapePoints()
{
	...
	if(m_params.escapeAnchorType && m_targetId)
	{
		...
		if(pTarget)
		{
			AutoAIObjectIter it(gEnv->pAISystem->GetAIObjectManager()->GetFirstAIObjectInRange(OBJFILTER_TYPE, m_params.escapeAnchorType, pTarget->GetWorldPos(), 400, true));
