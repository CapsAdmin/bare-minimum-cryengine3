...
namespace
{
	...
	ILINE bool DebugTurret()
	{
		return (g_pGameCVars->i_debug_turrets != 0);
	}
...
void CGunTurret::UpdateGoal(IEntity *pTarget, float deltaTime)
{
	...
	if(!IsTargetAimable(goalYaw, goalPitch))
	{
		if (DebugTurret())
		{
			CryLog("UpdateGoal: %s IsTargetAimable failed (yaw: %.2f, pitch: %.2f)", pTarget->GetName(), goalYaw, goalPitch);
		}
...
void CGunTurret::InternalStartFire(bool sec)
{
	...
	{
		CryLog("%s StartFire(%i)", GetEntity()->GetName(), sec ? 2 : 1);
	}
...
void CGunTurret::InternalStopFire(bool sec)
{
	if (g_pGameCVars->i_debug_turrets)
	{
		CryLog("%s StopFire(%i)", GetEntity()->GetName(), sec ? 2 : 1);
	}
