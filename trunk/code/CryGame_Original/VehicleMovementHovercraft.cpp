...
//------------------------------------------------------------------------
bool CVehicleMovementHovercraft::SetParams(const CVehicleParams &table)
{
	return true;
}
			...
			if (CVehicleParams thruster = thrusterTable.getChild(i))
			{
				...
				if (thruster.haveAttr("helper"))
				{
					if (IVehicleHelper *pHelper = m_pVehicle->GetHelper(thruster.getAttr("helper")))
					{
						...
						m_vecThrusters[i]->pos = pHelper->GetVehicleSpaceTranslation();
					}
				}
		...
		for (iter = m_vecThrusters.begin(); iter != m_vecThrusters.end(); ++iter)
		{
			...
			if (pThruster->hoverVariance > 0.f && bStartComplete)
			{
				...
				else
				{
					...
					hoverHeight += sinf(m_hoverTimer) * (pThruster->hoverVariance * pThruster->hoverHeight);
				}
			}
			...
			if (bThrusterUpdate)
			{
				...
				{
					if (gEnv->pPhysicalWorld->RayWorldIntersection(thrusterPos, -m_thrusterMaxHeightCoeff * (hoverHeight / cosAngle)*thrusterDir, objTypes, flags, &hit, 1, pSkip))
					{
						pThruster->hit = true;
...
//------------------------------------------------------------------------
void CVehicleMovementHovercraft::GetMemoryUsage(ICrySizer *s) const
{
	s->Add(*this);
...
//------------------------------------------------------------------------
void CVehicleMovementHovercraft::Serialize(TSerialize ser, EEntityAspects aspects)
{
...
//------------------------------------------------------------------------
void CNetworkMovementHovercraft::Serialize(TSerialize ser, EEntityAspects aspects)
{
