...
const char * CFeatureTester::GetActorInfoString(IActor * iActor, CryFixedStringT<256> & reply)
{
	if (iActor)
	{
		...
		if (spectatorMode != CActor::eASM_None)
		{
			...
			reply.append(" [SPECTATING mode=%d]", spectatorMode);
		}
	}
	else
	{
	...
	if (failureMessage && failureMessage[0])
	{
		...
		if (gameRules)
		{
			...
			if (level)
			{
				...
				actuallySendFailureMessage.append(g_pGame->GetIGameFramework()->IsGameStarted() ? " in progress," : " not in progress,");
