...
CVehicleMovementArcadeWheeled::CVehicleMovementArcadeWheeled()
{
	...
	m_surfaceFriction[0] = 1.0f;
	m_surfaceFriction[1] = 1.0f;
	m_surfaceFriction[2] = 1.0f;
	m_surfaceFriction[3] = 1.0f;
}
...
void CVehicleMovementArcadeWheeled::Reset()
{
	...
	if (pPhysics && m_numWheels > 0)
	{
		...
		for (int i = 0; i < numWheels; i++)
		{
			...
			if ((w->offset.x < 0.f) && ((+w->offset.y - w->offset.x) >= bestOffset[0]))
			{
				bestOffset[0] = +w->offset.y - w->offset.x;
				m_chassis.contactIdx0 = i;
			}

			if ((w->offset.x < 0.f) && ((-w->offset.y - w->offset.x) >= bestOffset[1]))
			{
				bestOffset[1] = -w->offset.y - w->offset.x;
				m_chassis.contactIdx1 = i;
			}

			if ((w->offset.x >= 0.f) && ((+w->offset.y + w->offset.x) >= bestOffset[2]))
			{
				bestOffset[2] = +w->offset.y + w->offset.x;
				m_chassis.contactIdx2 = i;
			}

			if ((w->offset.x >= 0.f) && ((-w->offset.y + w->offset.x) >= bestOffset[3]))
			{
				bestOffset[3] = -w->offset.y + w->offset.x;
				m_chassis.contactIdx3 = i;
			}
...
void CVehicleMovementArcadeWheeled::DebugDrawMovement(const float deltaTime)
{
	...
	{
		pRenderer->Draw2dLabel(5.f,   y, sizeL, color, false, "You are driving VehicleMovementStdWheeled 3");
		y += step2;
	}
...
#define SQR(x) ((x)*(x))
	...
	//===============================================
	{
		...
		for (int i = 0; i < numWheels; i++)
		{
			...
			if (w->bCanLock || lockAllWheels)
			{
				numWheelsThatCanHadnBrake++;
			}
		}
		...
		if (numWheelsThatCanHadnBrake)
		{
			handBrakeForce /= (float)numWheelsThatCanHadnBrake;
		}
		...
		for (int i = 0; i < numWheels; i++)
		{
			...
			if (m_wheelStatus[i].bContact)
			{
				w->contactNormal = w->contactNormal + m_wheelStatus[i].normContact;
			}
...
{
	if (g_pGameCVars->v_profileMovement == 0)
	{
		return;
	}
