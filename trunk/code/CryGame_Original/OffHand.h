...
class COffHand : public CWeapon
{
	...
	public:
		...
		ILINE void SetResetTimer(float t)
		{
			m_resetTimer = t;
		}
		ILINE int32 GetGrabType() const
		{
			return m_grabType;
		}
		...
		bool PerformPickUp();
		int  CanPerformPickUp(CActor *pActor, bool getEntityInfo = false);
		...
		//Offhand (for Player)
		void PerformThrowPlayer(int activationMode, EntityId throwableId, int oldFMId = -1, bool isLivingEnt = false);
		...
		void ForcePickUp(EntityId pickupId);

		inline EntityId GetPreHeldEntityId() const
		{
			return m_preHeldEntityId;
		}
	...
	private:
		...
		bool					m_npcWasDead;
		bool					m_startPickUp;
		float					m_grabbedNPCInitialHealth;
		bool          m_forceThrow;
