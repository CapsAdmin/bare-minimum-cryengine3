...
SSessionNames::SSessionName::SSessionName(CryUserID userId, const SCryMatchMakingConnectionUID &conId, const char* name, const uint8* userData, bool isDedicated)
{
	...
	memset(m_onlineData, 0, sizeof(m_onlineData));
	m_onlineDataCount = 0;
	m_recvOnlineDataCount = 0;
	m_onlineStatus = eOAS_Uninitialised;
	m_numGetOnlineDataRetries = 0;
#endif
...
#if !defined(_RELEASE)
#ifdef GAME_IS_CRYSIS2
	if (g_pGame->GetUI()->GetCVars()->menu_namesAsWs)
	{
...
//---------------------------------------
uint8 SSessionNames::SSessionName::GetReincarnations() const
{
	uint8 reincarnationAndVote = m_userData[eLUD_ReincarnationsAndVoteChoice];
	return (reincarnationAndVote & 0xF0) >> 4;
}

void SSessionNames::SSessionName::SetOnlineData(SCryLobbyUserData* pData, uint32 dataCount)
{
#if INCLUDE_DEDICATED_LEADERBOARDS
	m_onlineDataCount = dataCount;
	memcpy(m_onlineData, pData, min(sizeof(m_onlineData), sizeof(SCryLobbyUserData) * dataCount));
#endif
}
...
void SSessionNames::Tick(float dt)
{
	...
	for (uint32 i = 0; i < numPlayers; ++ i)
	{
		...
		if (pSessionName->m_conId == CryMatchMakingInvalidConnectionUID)
		{
			...
			if(pSessionName->m_timeWithoutConnection >= g_pGameCVars->gl_maxSessionNameTimeWithoutConnection)
			{
