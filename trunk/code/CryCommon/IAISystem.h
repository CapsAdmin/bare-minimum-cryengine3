...
#define BAI_AREA_FILE_VERSION_WRITE 23
...
struct SAIStimulus
{
	SAIStimulus() : sourceId(0), targetId(0), pos(ZERO), dir(ZERO), radius(0), type(0), subType(0), flags(0)
	{
	}
...
UNIQUE_IFACE struct IAISystem
{
	...
	virtual void GetStats(AIStats& stats) = 0;
	...
	virtual void FlushSystem() = 0;
	virtual void FlushSystemNavigation() = 0;
	...
	// AI Objects Logger
	virtual bool	IsLoggingAIActor(const IAIActor* pAIActor) const = 0;
	virtual void	LogAIActorDebugEvent(const IAIActor* pAIActor, EAIDebugEvent eAIDebugEvent, const char* szString) const = 0;

	// AI Recorder
	virtual void	GetAIRecorderContext(SAIRecorderContext* &pContext) = 0;
	...
	virtual void GetDetectionLevels(IAIObject *pObject, SAIDetectionLevels& levels) const = 0;
	virtual void RegisterDetectionLevels(IAIObject *pObject, const SAIDetectionLevels& levels) const = 0;
