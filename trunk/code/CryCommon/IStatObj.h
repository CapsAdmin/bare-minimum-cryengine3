...
struct IStreamable
{
  ...
  bool UpdateStreamingPrioriryLowLevel(float fImportance, int nRoundId, bool bFullUpdate)
  {
    ...
    if(m_arrUpdateStreamingPrioriryRoundInfo[0].nRoundId != nRoundId)
    {
      ...
      m_arrUpdateStreamingPrioriryRoundInfo[0].fSumImportance = fImportance;
    }
    else
    {
      m_arrUpdateStreamingPrioriryRoundInfo[0].fSumImportance += fImportance;
...
UNIQUE_IFACE struct IStatObj : public IStreamable
{
	...
	struct SStatistics
	{
		...
		SStatistics() : pTextureSizer(0),pTextureSizer2(0),nVertices(0),nIndices(0),nMeshSize(0),nMeshSizeLoaded(0),nNumRefs(0),
			nPhysProxySize(0),nPhysPrimitives(0),nDrawCalls(0),nLods(0),nSubMeshCount(0),bSplitLods(false) { ZeroStruct(nIndicesPerLod); }
	};
...
//DOC-IGNORE-END
	...
	virtual int PhysicalizeSubobjects(IPhysicalEntity *pent, const Matrix34 *pMtx, float mass,float density=0.0f, int id0=0, 
		strided_pointer<int> pJointsIdMap=0, const char *szPropsOverride=0) = 0;
	// Summary:
