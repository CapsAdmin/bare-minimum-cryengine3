...
#define  FSL_VBIBPUSHDOWN     0x200  // Push down from vram on demand if target architecture supports it, used internally
#define  FSL_DIRECT           0x400  // Access VRAM directly if target architecture supports it, used internally
...
UNIQUE_IFACE struct IRenderMesh
{
  ...
  virtual bool UpdateIndices(const uint16 *pNewInds, int nInds, int nOffsInd, uint32 copyFlags, bool requiresLock=true) = 0;
  ...
  virtual const PodArray<std::pair<int,int> > * GetTrisForPosition(const Vec3 & vPos, IMaterial * pMaterial) = 0;
	...
	{
		const QuatD*	aBoneLocs;
		const uint8*	aBoneRemap;
		const UCol*		aVertBones;
		const f32*		aShapeDeform;
	};
  ...
  virtual void SetREUserData(float * pfCustomData, float fFogScale=0, float fAlpha=1) = 0;
	virtual void DrawImmediately() = 0;
