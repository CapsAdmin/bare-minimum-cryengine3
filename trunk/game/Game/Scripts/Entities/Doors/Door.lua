...
Door = 
{
	Properties = 
	{
		...
		fileModel = "objects/structures/buildings/fishing_houses/fishing_house_door_a.cgf",
...
function Door:IsUsable(user)
	...
	if (not user or self.Properties.bLocked == 1) then
		return 0;
	end
...
function Door:IsUsableMsgChanged()
	local ret = self.msgChanged;
	self.msgChanged = 0;
	return ret;
end
